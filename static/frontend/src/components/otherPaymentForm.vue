<template>
    <div class="container" style="overflow: auto">
        ...
        <div class="d-flex flex-row flex-wrap" >
            ...
            <select class="form-select m-3" id="memberName" v-model="receiptData.loanAccount" style="width: max-content;" v-if="receiptData.transactionType === '1'">
                <option v-for="account in Account_list" :value="account.id">{{account.bank_name}}-{{ account.account_number }}</option>
            </select>
            ...
            <div class="mb-3 col-md-6 " v-if="receiptData.transactionType == 1">
                ...
                <input type="date" class="form-control  mx-3" id="depositDate" v-model="receiptData.interestDate">
            </div>
...
export default {
    ...
    methods : {
        submitForm() {  
            axios.post('/api/v1/otherReceipt', this.receiptData ,  { headers:{ 'Token': localStorage.getItem('token') } } ).then((response) => {
