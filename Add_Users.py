...
def create_users(): 
    ...
    if not user_datastore.find_user(email='manager@shgplus.in'):
        user_datastore.create_user(id=2, first_name='Mayur', last_name='Kharade', email='manager@shgplus.in', password=hash_password('password'), last_login=datetime.datetime.now(), roles=['manager'], date_created=datetime.datetime.now())
